...
instance Functor (BiList fixed) where
    ...
    fmap f BiNil = BiNil
    fmap f (BiCons fixed a b) = BiCons fixed (f a) (fmap f b)
    fmap f (ACons fixed a) = ACons fixed (fmap f a)
    fmap f (BCons a b) = BCons (f a) (fmap f b)

instance Foldable (BiList fixed) where
    foldr :: (a -> b -> b) -> b -> BiList fixed a -> b
    foldr f b BiNil = b
    foldr f b (BiCons fixed a c) = f a (foldr f b c)
    foldr f b (ACons fixed a) = foldr f b a
    foldr f b (BCons a c) = f a (foldr f b c)

    foldMap :: (Monoid m) => (a -> m) -> BiList fixed a -> m
    foldMap f BiNil = mempty
    foldMap f (BiCons fixed a b) = f a <> foldMap f b
    foldMap f (ACons fixed a) = foldMap f a
    foldMap f (BCons a b) = f a <> foldMap f b
...
instance Traversable (BiList fixed) where
    ...
    traverse f (BiCons fixed a b) = BiCons <$> pure fixed <*> f a <*> traverse f b
    traverse f (ACons fixed a) = ACons <$> pure fixed <*> traverse f a
    traverse f (BCons a b) = BCons <$> f a <*> traverse f b 
...
mapL f BiNil = BiNil
mapL f (BiCons a fixed b) = BiCons (f a) fixed (mapL f b)
mapL f (ACons a b) = ACons (f a) (mapL f b)
mapL f (BCons fixed a) = BCons fixed (mapL f a)

foldL f BiNil = mempty
foldL f (BiCons a fixed b) = f a <> foldL f b
foldL f (ACons a b) = f a <> foldL f b
foldL f (BCons a b) = foldL f b

separate :: BiList a b -> ([a], [b])
separate BiNil = ([], []) 
separate (BiCons a b c) = 
    case separate c of 
        (d, e) -> (a : d, b : e)
separate (ACons a b) =
    case separate b of 
...
printLR bilist = printSeparate (separate bilist)
    where 
        printSeparate :: (Show a, Show b) => ([a], [b]) -> IO ()
        printSeparate (a, b) = do
            printHelp a 
            putStrLn ""
            printHelp b
            putStrLn ""
...
data Pattern = ByValue Exp | Wildcard | Named String
  deriving (Eq, Show)
...
data Statement
  ...
  | PatternMatch Exp [(Pattern, [Statement])]
  deriving (Eq, Show)
...
statement = sIf <|> sWhile <|> sAssign <|> sPatternMatch
...
sPatternMatch :: Parser Statement
sPatternMatch = do
    pKeyword "match"
    p <- pExp
    pKeyword ":"
    s <- patternHelp
    pKeyword "end"
    return $ PatternMatch p s
    where 
        patternHelp :: Parser [(Pattern, [Statement])]
        patternHelp =
            (
                do
                    string' "_"
                    ...
                    case p of
                        Var x -> 
                            ...
                            (
                                do
                                    rest <- patternHelp
                                    return ((Named x, s): rest)
                                ) <|> return [(Named x, s)]
                        _ -> do
                            (
                                do
                                    rest <- patternHelp
                                    return ((ByValue p, s): rest)
                                ) <|>  
                                    do
                                        return [(ByValue p, s)]
                )
...
evalStatement (PatternMatch e statements) = do
    env <- get
    exp <- evalExp e env
    undefined
